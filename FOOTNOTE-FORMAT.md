...
CREATE TABLE chunks (
  ...
  assertion  TEXT NOT NULL DEFAULT 'unspecified',  -- Chunk-level clinical assertion status (Phase 2)
  findings   TEXT NOT NULL DEFAULT '[]'  -- JSON array of per-finding assertions (Phase 2, bullet 2)
);
...
| `findings` | TEXT | JSON array of per-finding assertions for curated high-risk clinical findings (issue #15, Phase 2 bullet 2): `[{ "finding", "assertion", "evidence" }]`. `[]` when none. Concept-anchored, so a mixed sentence asserts each finding independently. |
...
| `1.1` | Added `chunks.assertion` (+ index) and `chunks.findings` columns (clinical assertion metadata, issue #15 Phase 2). Additive/backward-compatible: older readers ignore the columns; older indexes are forward-migrated via `ALTER TABLE` on open. |
